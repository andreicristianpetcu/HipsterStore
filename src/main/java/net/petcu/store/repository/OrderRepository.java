...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(
        value = "select jhiOrder from Order jhiOrder left join fetch jhiOrder.user left join fetch jhiOrder.discount",
        countQuery = "select count(jhiOrder) from Order jhiOrder"
    )
    Page<Order> findAllWithToOneRelationships(Pageable pageable);

    @Query("select jhiOrder from Order jhiOrder left join fetch jhiOrder.user left join fetch jhiOrder.discount")
    List<Order> findAllWithToOneRelationships();

    @Query("select jhiOrder from Order jhiOrder left join fetch jhiOrder.user left join fetch jhiOrder.discount where jhiOrder.id =:id")
