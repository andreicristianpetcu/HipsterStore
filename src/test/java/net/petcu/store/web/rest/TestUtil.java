...
public final class TestUtil {
    ...
    /**
     * Verifies the equals/hashcode contract on the domain object or record.
     */
    public static <T> void equalsVerifier(Class<T> clazz) throws Exception {
        if (clazz.isRecord()) {
            // Handle records
            T recordObject1 = createRecordInstance(clazz);
            assertThat(recordObject1.toString()).isNotNull();
            assertThat(recordObject1).isEqualTo(recordObject1);
            assertThat(recordObject1).hasSameHashCodeAs(recordObject1);

            // Test with an instance of another class
            Object testOtherObject = new Object();
            assertThat(recordObject1).isNotEqualTo(testOtherObject);
            assertThat(recordObject1).isNotEqualTo(null);

            // Test with another instance with same constructor args
            T recordObject2 = createRecordInstance(clazz);
            assertThat(recordObject1).isEqualTo(recordObject2);
            assertThat(recordObject1).hasSameHashCodeAs(recordObject2);
        } else {
    ...
    /**
     * Creates a record instance with default values based on component types.
     */
    private static <T> T createRecordInstance(Class<T> recordClass) throws Exception {
